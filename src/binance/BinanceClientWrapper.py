...
class BinanceClientWrapper(ExchangeClientWrapper):
    ...
    def get_trades(self, symbol, start_dt):
        ...
        df = pd.DataFrame(self.client.get_my_trades(symbol=symbol, fromId=0, limit=1))
        if len(df) == 0:
            return df_trades
        first_id = int(df["id"].item())
        first_dt = pd.to_datetime(df["time"].item(), unit="ms")
        df_last = pd.DataFrame(self.client.get_my_trades(symbol=symbol, limit=1))
        ...
        while first_dt < start_dt:
            ...
            df = pd.DataFrame(self.client.get_my_trades(symbol=symbol, fromId=next_id, limit=1))
            if len(df) > 0:
                next_id = df["id"].item()
        ...
        while max_id < last_id:
            df = pd.DataFrame(self.client.get_my_trades(symbol=symbol, fromId=max_id))
            if df_trades.empty:
                df_trades = df
            else:
