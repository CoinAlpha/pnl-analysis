...
class AscendexClientWrapper(ExchangeClientWrapper):
    ...
    def symbol_info(self,trading_pair):
        ...
        raise Exception("Trading pair not valid for ascendex")
    
    def get_trades(self, symbol, start_date,account='cash'):
        data = self.client.getHistOrders(account=account,symbol=symbol, startTime=start_date)
        ...
        while True:
            data = self.client.getHistOrders(account=account,symbol=symbol, startTime=start_date,seqNum=seqNum)
    ...
    def format_data(self,df):
        ...
        df = df.astype({'price':'float64','qty':'float64','commission':'float64','commissionAssetUsdPrice':'float64'})
